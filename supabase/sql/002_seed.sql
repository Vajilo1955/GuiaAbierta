...
with project as (
  insert into public.guia_projects (name, slug, short_description, cover_image_url, sort_order)
  values (
    'Centro Historico',
    'centro-historico',
...
plaza as (
  insert into public.guia_elements (
    ...
    main_image_url, maps_url, featured, sort_order
  )
  select
    project.id,
    cat_monumento.id,
...
ayuntamiento as (
  insert into public.guia_elements (
    project_id, category_id, title, slug, short_description, long_description,
    main_image_url, maps_url, sort_order
  )
  select
    project.id,
    cat_servicio.id,
